...
import * as myExtension from '../extension';  // パスを修正

suite('Comment Translate & Chat with Claude Extension Test Suite', () => {
    vscode.window.showInformationMessage('テストを開始します。');

    test('拡張機能が存在すること', () => {
        assert.ok(vscode.extensions.getExtension('YourPublisherName.comment-translate-claude'));
    });

    test('拡張機能がアクティベートできること', async () => {
        const ext = vscode.extensions.getExtension('YourPublisherName.comment-translate-claude');
        await ext?.activate();
        assert.ok(true);
    });
    ...
    test('Hover Provider が登録されていること', async () => {
        ...
        // ダミーのTextDocumentを作成
        const dummyDoc = await vscode.workspace.openTextDocument({ content: '// Test comment', language: 'plaintext' });
    ...
    test('コメント判定機能が正しく動作すること', () => {
        ...
        assert.strictEqual(myExtension.isComment(nonCommentDocument, position), false);
    });
